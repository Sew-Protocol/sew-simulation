...
with SimulationClient(port=50051) as client:
    with managed_session(client, [
        ...
        # Register stake
        client.step(sid, {'seq': -1, 'time': 1000, 'agent': 'resolver', 'action': 'register_stake', 
                             'params': {'amount': 10000}})
        ...
        # Execute resolution  
        r = client.step(sid, {'seq': 2, 'time': 1120, 'agent': 'resolver', 'action': 'execute_resolution', 
                              'params': {'workflow_id': wf_id, 'is_release': False, 'resolution_hash': '0xhash'}})
        print(f'execute_resolution: {r["result"]}')
        
        # Propose fraud slash
        r = client.step(sid, {'seq': 3, 'time': 1180, 'agent': 'governance', 'action': 'propose_fraud_slash', 
